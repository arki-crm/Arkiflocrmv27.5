...
const REFUND_MODES = [
  ...
  { value: 'adjustment', label: 'Adjust in Next Invoice' },
  { value: 'no_refund', label: 'No Refund' }
];

const ITEM_DISPOSITIONS = [
  { value: 'returned_to_vendor', label: 'Return to Vendor' },
  ...
  { value: 'reused_other_project', label: 'Reused in Other Project' },
...
const REFUND_STATUSES = {
  ...
  adjusted: { label: 'Adjusted', color: 'bg-purple-100 text-purple-700', icon: Repeat },
...
export default function SalesReturns() {
  ...
  const fetchReturns = async () => {
    try {
      ...
      const response = await axios.get(`${API}/finance/sales-returns?${params}`, {
  ...
  const handleSubmit = async () => {
    ...
    try {
      setCreating(true);
      await axios.post(`${API}/finance/sales-returns`, formData, {
        withCredentials: true
      });
      ...
      toast.error(err.response?.data?.detail || 'Failed to create sales return');
    } finally {
      setCreating(false);
    }
  };
    ...
    <div className="p-6 max-w-7xl mx-auto space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        {hasPermission('finance.invoices.create') && (
          <Button onClick={() => setShowCreateModal(true)} data-testid="create-sales-return-btn">
              ...
              <table className="w-full">
                <thead className="bg-slate-50 border-b">
                  <tr>
                    ...
                    <th className="text-center p-3 text-xs font-medium text-slate-500">Replacement</th>
                  </tr>
                </thead>
                <tbody className="divide-y">
                  {returns.map((ret) => {
                    ...
                    return (
                      <tr key={ret.return_id} className="hover:bg-slate-50" data-testid={`sales-return-row-${ret.return_id}`}>
                        <td className="p-3">
                          <span className="font-mono text-sm text-blue-600">{ret.return_id}</span>
                        </td>
                        <td className="p-3 text-sm">{ret.return_date}</td>
                        <td className="p-3 text-sm">{ret.customer_name || 'Unknown'}</td>
                        ...
                        <td className="p-3 text-center">
                          {ret.requires_replacement ? (
                            <Badge className={ret.replacement_status === 'delivered' ? 'bg-green-100 text-green-700' : 'bg-amber-100 text-amber-700'}>
                              {ret.replacement_status === 'delivered' ? 'Delivered' : 'Pending'}
                            </Badge>
                          ) : (
            ...
            <div>
                ...
                <SelectContent>
                  {invoices.map(inv => (
                    <SelectItem key={inv.invoice_id} value={inv.invoice_id}>
                      {inv.invoice_number || inv.invoice_id} - {inv.customer_name} - {formatCurrency(inv.total_amount)}
                    </SelectItem>
                  ))}
                </SelectContent>
              </Select>
              {selectedInvoice && (
                <p className="text-xs text-slate-500 mt-1">
                  Invoice Total: {formatCurrency(selectedInvoice.total_amount)} | 
            ...
            <div className="space-y-3 border rounded-lg p-4 bg-slate-50">
              <div className="flex items-center space-x-2">
                <Checkbox
                  ...
                  checked={formData.requires_replacement}
                  onCheckedChange={(checked) => setFormData(prev => ({ ...prev, requires_replacement: checked }))}
                />
                <Label htmlFor="requires_replacement" className="cursor-pointer">Requires Replacement</Label>
              </div>
              {formData.requires_replacement && (
                <div className="grid grid-cols-2 gap-4 mt-2">
                  <div>
                    <Label className="text-xs">Replacement Item</Label>
                    <Input
                      placeholder="Item to replace with"
                      value={formData.replacement_item}
                      onChange={(e) => setFormData(prev => ({ ...prev, replacement_item: e.target.value }))}
                    />
                  </div>
                  <div>
                    ...
                    <Input
                      ...
                      value={formData.replacement_notes}
                      onChange={(e) => setFormData(prev => ({ ...prev, replacement_notes: e.target.value }))}
                    />
                  </div>
                </div>
              )}
