...
class TestGeneralLedgerAccounts:
    ...
    def test_employees_not_empty(self):
    ...
    def test_accounts_have_valid_structure(self):
    ...
    def test_control_accounts_exist(self):
        """Test that control accounts are returned"""
        response = self.session.get(f"{BASE_URL}/api/finance/general-ledger/accounts")
        data = response.json()
        
        assert len(data["control_accounts"]) > 0, "No control accounts found"
        print(f"✓ Found {len(data['control_accounts'])} control accounts")
...
class TestGeneralLedgerFiltering:
    ...
    def test_filter_by_customer(self):
        ...
        customers = accounts_response.json()["party_filters"]["customers"]
    ...
    def test_filter_by_vendor(self):
        ...
        accounts_response = self.session.get(f"{BASE_URL}/api/finance/general-ledger/accounts")
        vendors = accounts_response.json()["party_filters"]["vendors"]
