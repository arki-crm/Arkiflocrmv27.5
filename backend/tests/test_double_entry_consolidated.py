"""
Test Suite: Double-Entry Accounting P0 Fix - Consolidated
==========================================================
Tests the P0 fix for strict atomic, deterministic double-entry flows.

This test suite verifies:
1. Liability Creation creates GL entries
2. Liability Settlement creates exactly 2 GL entries
3. Invoice Payment creates exactly 2 GL entries
4. Idempotency: duplicate requests are rejected with 409
5. Settlement updates liability status correctly
6. Validation: amount exceeds remaining, closed liability

Uses a single session to avoid rate limiting issues.
"""
...
def get_session():
    ...
    if _session is None:
        ...
        _session_token = login_data.get("session_token")
        if _session_token:
            _session.headers.update({"Authorization": f"Bearer {_session_token}"})
    
    return _session


def get_valid_account(session):
    """Helper to get a valid bank/cash account for testing"""
    accounts_response = session.get(f"{BASE_URL}/api/accounting/accounts")
...
class TestDoubleEntryAccountingP0Fix:
    ...
    def setup(self):
        ...
        try:
            ...
            pytest.skip(f"Session setup failed: {e}")
        yield
    
    # ============ TEST 1: Liability Creation ============
    def test_01_liability_creation_success(self):
        """
    ...
    def test_02_liability_settlement_creates_double_entry(self):
        """
        ...
        # Get a valid account
        test_account = get_valid_account(self.session)
    ...
    def test_03_full_settlement_closes_liability(self):
        """
    ...
    # ============ TEST 4: Idempotency - different timestamps create different txns ============
    def test_04_idempotency_different_timestamp_creates_new_pair(self):
        """
        TEST 4: Verify that same settlement with different timestamp creates new pair
        """
        # Create a liability
        unique_id = uuid.uuid4().hex[:8]
        liability_payload = {
    ...
    def test_05_settlement_amount_exceeds_remaining_rejected(self):
        """
    ...
    def test_06_settlement_on_closed_liability_rejected(self):
        """
    ...
    def test_07_invoice_payment_creates_double_entry(self):
        """
        ...
        # Record a payment
        payment_amount = min(100, remaining)  # Small amount to avoid depleting test data
        unique_id = uuid.uuid4().hex[:8]
        payment_payload = {
        ...
        assert payment_data.get("success") == True, "Payment should succeed"
        assert "transaction_id" in payment_data, "Response should contain transaction_id"
        
        print(f"  Payment transaction: {payment_data.get('transaction_id')}")
...
class TestExistingTestData:
    ...
    def test_08_existing_liability_lia_e1297573(self):
        """
    ...
    def test_09_existing_invoice_exec_9ce599df7995(self):
        """
